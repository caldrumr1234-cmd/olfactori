// WardrobeTab.jsx — Wardrobe Mode: weather, suggestions, heatmap, wear log
...
const css = `
  ...
  .heatmap-controls { display: flex; align-items: center; gap: 10px; margin-bottom: 16px; flex-wrap: wrap; }
  .heatmap-year-btn { background: var(--bg3); border: 1px solid var(--border); border-radius: 6px; color: var(--text2); padding: 5px 12px; font-size: 12px; cursor: pointer; transition: all 0.15s; font-family: 'DM Sans', sans-serif; }
  .heatmap-year-btn.active { border-color: var(--gold); color: var(--gold); background: var(--gold-dim); }
  .heatmap-year-btn:hover { border-color: var(--border2); color: var(--text); }
  .heatmap-year-input { background: var(--bg3); border: 1px solid var(--border); border-radius: 6px; color: var(--text); padding: 5px 10px; font-size: 12px; width: 70px; outline: none; font-family: 'DM Sans', sans-serif; transition: border-color 0.15s; }
  .heatmap-year-input:focus { border-color: var(--gold); }
  .heatmap-go-btn { background: var(--gold); border: none; border-radius: 6px; color: #0c0c0f; padding: 5px 12px; font-size: 12px; cursor: pointer; font-family: 'DM Sans', sans-serif; font-weight: 600; }
  .heatmap-wrap { overflow-x: auto; padding-bottom: 8px; }
  .heatmap-grid { display: flex; gap: 3px; min-width: max-content; }
  .heatmap-week { display: flex; flex-direction: column; gap: 3px; }
  .heatmap-day { width: 12px; height: 12px; border-radius: 2px; background: var(--bg3); border: 1px solid var(--border); transition: transform 0.1s; cursor: default; position: relative; }
  .heatmap-day:hover { transform: scale(1.4); z-index: 10; }
  .heatmap-day.worn-1 { background: rgba(201,168,76,0.3); border-color: rgba(201,168,76,0.4); }
  .heatmap-day.worn-2 { background: rgba(201,168,76,0.55); border-color: rgba(201,168,76,0.6); }
  .heatmap-day.worn-3 { background: rgba(201,168,76,0.8); border-color: rgba(201,168,76,0.85); }
  .heatmap-day.worn-4 { background: var(--gold); border-color: var(--gold2); }
  .heatmap-total { font-size: 12px; color: var(--text3); margin-top: 8px; }
  .monthly-chart { display: flex; align-items: flex-end; gap: 8px; height: 80px; margin-bottom: 8px; }
  .monthly-col { display: flex; flex-direction: column; align-items: center; gap: 4px; flex: 1; }
  .monthly-bar-wrap { display: flex; flex-direction: column; justify-content: flex-end; height: 64px; width: 100%; }
  .monthly-bar { width: 100%; background: var(--gold-dim); border: 1px solid rgba(201,168,76,0.3); border-radius: 3px 3px 0 0; min-height: 0; transition: background 0.15s; }
  .monthly-bar:hover { background: var(--gold); }
  .monthly-label { font-size: 9px; color: var(--text3); }
  .monthly-cnt { font-size: 10px; color: var(--text2); }
...
// ── HEATMAP ───────────────────────────────────────────────────
function WearHeatmap({ year }) {
  const [data, setData] = useState(null);

  useEffect(() => {
    fetch(`${API}/insights/wear_heatmap?year=${year}`)
      .then(r => r.json()).then(setData).catch(() => {});
  }, [year]);

  if (!data) return <div style={{color:"var(--text3)",fontSize:12}}>Loading heatmap...</div>;

  const dayMap = {};
  (data.days || []).forEach(d => { dayMap[d.worn_date] = d.cnt; });
  const total = (data.days || []).reduce((s, d) => s + d.cnt, 0);
  ...
  const getClass = (cnt) => {
    if (!cnt) return "";
    if (cnt === 1) return "worn-1";
    if (cnt === 2) return "worn-2";
    if (cnt === 3) return "worn-3";
    return "worn-4";
  };
  ...
  return (
    <div>
      <div className="heatmap-wrap">
        <div style={{display:"flex",gap:"3px",minWidth:"max-content",marginBottom:4}}>
          {weeks.map((_, wi) => {
            const label = Object.entries(monthPositions).find(([,pos]) => pos === wi);
            return (
              <div key={wi} style={{width:12,fontSize:9,color:"var(--text3)",textAlign:"center",flexShrink:0}}>
                {label ? MONTH_NAMES[parseInt(label[0])] : ""}
              </div>
            );
          })}
        </div>
        <div className="heatmap-grid">
          {weeks.map((week, wi) => (
            <div key={wi} className="heatmap-week">
              {week.map((d, di) => {
                const dateStr = fmt(d);
                const cnt = dateStr ? (dayMap[dateStr] || 0) : 0;
                return (
                  <div key={di} className={`heatmap-day ${d ? getClass(cnt) : ""}`}
                    style={!d ? {opacity:0} : {}}
                    title={dateStr ? `${dateStr}: ${cnt} wear${cnt !== 1 ? "s" : ""}` : ""}
                  />
                );
              })}
            </div>
      ...
      <div className="heatmap-total">{total} wear{total !== 1 ? "s" : ""} logged in {year}</div>
    </div>
  );
}
...
function MonthlyChart({ year }) {
  ...
  useEffect(() => {
    fetch(`${API}/insights/wear_by_month?year=${year}`)
      .then(r => r.json()).then(setData).catch(() => {});
  }, [year]);

  if (!data) return <div style={{color:"var(--text3)",fontSize:12}}>Loading...</div>;
    ...
    <div className="monthly-chart">
      {months.map((m, i) => (
        <div key={m.month} className="monthly-col">
          <div className="monthly-cnt">{m.cnt > 0 ? m.cnt : ""}</div>
          <div className="monthly-bar-wrap">
            <div className="monthly-bar"
              style={{height:`${Math.max(0, Math.round((m.cnt/maxCnt)*56))}px`}}
              title={`${MONTH_NAMES[i]}: ${m.cnt} wears`}
            />
          </div>
          <div className="monthly-label">{MONTH_NAMES[i]}</div>
        </div>
      ))}
    </div>
  );
...
export default function WardrobeTab({ onOpenFrag }) {
  ...
  const [heatYear, setHeatYear]   = useState(thisYear);
  ...
  const goYear = () => { const y = parseInt(yearInput); if (y > 2000 && y <= thisYear) setHeatYear(y); };
      ...
      <div className="wardrobe-wrap">
        ...
        <div className="wardrobe-location">
          ...
          <input className="wardrobe-location-input" value={cityInput}
            ...
            onKeyDown={e => e.key === "Enter" && updateCity()} placeholder="City, State" />
        ...
        {!results && <>
          ...
          <div className="occasion-grid">
            {OCCASIONS.map(o => (
              <button key={o.id} className={`occasion-btn ${occasion===o.id?"active":""}`} onClick={() => setOccasion(o.id)}>
                <span className="occasion-icon">{o.icon}</span>{o.label}
              </button>
            ))}
          </div>
          <button className="wardrobe-suggest-btn" onClick={getSuggestions} disabled={!occasion||loading||weatherLoading}>
        ...
        {results && <>
          ...
          <div className="wardrobe-results">
            {results.items.map((frag, i) => (
              <div key={frag.id} className={`wardrobe-result-card ${i===0?"primary":""}`} onClick={() => onOpenFrag && onOpenFrag(frag)}>
                <div className="wardrobe-result-img">
                  {imgSrc(frag) ? <img src={imgSrc(frag)} alt="" onError={e=>{e.target.style.display="none";}} /> : <span style={{fontSize:28}}>🧴</span>}
                </div>
        ...
        {/* HEATMAP */}
        <div className="wardrobe-section-title">Wear Activity</div>
        <div className="heatmap-controls">
          <button className={`heatmap-year-btn ${heatYear===thisYear?"active":""}`} onClick={() => { setHeatYear(thisYear); setYearInput(String(thisYear)); }}>{thisYear}</button>
          <button className={`heatmap-year-btn ${heatYear===lastYear?"active":""}`} onClick={() => { setHeatYear(lastYear); setYearInput(String(lastYear)); }}>{lastYear}</button>
          <input className="heatmap-year-input" value={yearInput} onChange={e => setYearInput(e.target.value)} onKeyDown={e => e.key==="Enter" && goYear()} placeholder="Year" />
          <button className="heatmap-go-btn" onClick={goYear}>Go</button>
        </div>
        <WearHeatmap key={heatYear} year={heatYear} />

        <hr className="wardrobe-divider" />

        {/* MONTHLY */}
        <div className="wardrobe-section-title">Month by Month · {heatYear}</div>
        <MonthlyChart key={`m-${heatYear}`} year={heatYear} />
