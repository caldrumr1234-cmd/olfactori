...
# ── NEW RELEASE RADAR ─────────────────────────────────────────
@router.get("/meta/new_releases")
def new_releases(year: int = None, db = Depends(get_db)):
    """
    For each unique brand in the collection, search Fragella and return
    releases from the given year not already owned.
    """
    import datetime
    if not year:
        year = datetime.datetime.now().year

    # Get all fragrances in collection for dedup
    rows = db.execute("SELECT brand, name FROM fragrances").fetchall()
    owned_keys = set()
    brands = set()
    for r in rows:
        ...
        for brand in sorted(brands):
            ...
            try:
                ...
                for item in items:
                    ...
                    # Must be from correct year and from a brand we own
                    if item_year != year:
                        continue
                    if item_brand not in brands:
                        continue

                    key = f"{item_brand}||{item_name}"
                    if key in seen_keys:
                        continue
                    seen_keys.add(key)
                    ...
                    results.append({
                        ...
                        "name":            item.get("Name", ""),
                        "year":            item_year,
                        "image_url":       item.get("Image URL") or item.get("image_url") or "",
                        "fragrantica_url": item.get("Fragrantica URL") or item.get("fragrantica_url") or "",
                        "concentration":   item.get("Concentration") or "",
                        "gender":          item.get("Gender") or "",
                        "in_collection":   key in owned_keys,
                    })
            except Exception:
                continue

    results.sort(key=lambda x: (x["brand"], x["name"]))
    return {"year": year, "items": results}
